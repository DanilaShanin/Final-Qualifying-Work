...
class Screensaver:
    ...
    def _set_canvas_size(self, size):
        self._canvas.configure(height=size[0], width=size[1])
    ...
    def _window_update_thread(self, shared_list, frame_amount, delay):
        sleep(1)  # Ждем загрузки нескольких кадров

        i = 0
        while self._working:
            image = ImageTk.PhotoImage(shared_list[i])  # Конвертируем изображения из формата библиотеки Pillow в формат, который может понять Tkinter
...
def destroy_root():
    #Функция для закрытия загрузочного окна
    root.destroy()
